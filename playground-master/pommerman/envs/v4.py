...
class Pomme(gym.Env):
    ...
    def make_board(self):
        ...
        for i in range(4, size - 4):
            self._board[1, i] = 0
            self._board[size - i - 1, 1] = 0
            self._board[size - 2, size - i - 1] = 0
            self._board[size - i - 1, size - 2] = 0  
        ######################################################################      
    ...
    def _get_rewards(self):
        ...
        if self.training_agent is not None :
            ...
            #     self.alive_enemy_num = len(alive_ids)-1
            #     return [100 - len(alive_ids)*20]*4
            reward = 0
            pos = np.where(self._board == self.training_agent+10)
            row = pos[0][0]
            col = pos[1][0]
            # if (row,col) in self.visited and self.observations[self.training_agent]['bomb_life'][row][col] < 9:
            #     reward -= 0.2
            for i in range(-1,2):
                for j in range(-1, 2):
                    erow, ecol = row + i, col + j
                    if (erow < 0 or ecol < 0 or erow > 10 or ecol > 10):
                        continue

                    ch = self._board[erow][ecol]
                    if ch >= 10 and ch != self.training_agent+10:
                        reward += 0
                        # if self.observations[self.training_agent]['bomb_life'][row][col] != 0:
                        #     reward += 0.1
    ...
    def in_range_of_bomb(self, _board, _bombs, _bomb_life):
        pos = np.where(_board == self.training_agent+10)
        if len(pos[0]) > 0:
            x = pos[0][0]
            y = pos[1][0]

        radius = 5
        # board = self.observations[self.training_agent]['board'].copy()
        # bombArray = self.observations[self.training_agent]['bomb_blast_strength'].copy()
        # bombTimeArray = self.observations[self.training_agent]['bomb_life'].copy()

        board = _board.copy()
        bombArray = _bombs.copy()
        bombTimeArray = _bomb_life.copy()
    ...
    def step(self, actions):
        ...
        if self.training_agent is not None :
            ...
            if len(pos[0]) > 0 and len(alive_now[0]) > 0:
                ...
                if self.in_range_of_bomb(self.observations[self.training_agent]['board'], self.observations[self.training_agent]['bomb_blast_strength'], self.observations[self.training_agent]['bomb_life']):
                    reward[self.training_agent] -= 4
                elif self.in_range_of_bomb(pre_board, pre_bombs, pre_bomb_life):
                    reward[self.training_agent] += 4
                ...
                elif self._board[row + x[actions[self.training_agent]]][col + y[actions[self.training_agent]]] != 0:
                    reward[self.training_agent] -= 1

                for i in range(1, 5):
                    erow, ecol = row + x[i], col + y[i]
                    if (erow < 0 or ecol < 0 or erow > 10 or ecol > 10):
                        continue

                    ch = self._board[erow][ecol]
                    if ch >= 10 and ch != self.training_agent+10 and actions[self.training_agent] == 5:
                        reward[self.training_agent] += 4
        ...
        self._step_count += 1
        # print(reward[self.training_agent])
