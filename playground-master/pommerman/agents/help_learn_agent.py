...
class HelpLearnAgent(BaseAgent):

    def __init__(self, *args, **kwargs):
        super(HelpLearnAgent, self).__init__(*args, **kwargs)
    ...
    def act(self, obs, action_space):
        ...
        def attack_condition():
            if self._maybe_bomb(
                    ...
                    and enemy_within_range(4):
                return True
            return False


        def direction_to_enemy():
            dire = self._near_enemy(my_position, items, dist, prev, enemies, 4)
            if dire is None:
                return 0
            return dire
        ...
        if evade_condition(5, my_position):
            ...
            direction = mcts.MCTS_search(obs, 0)
                # #print("evade", direction)
                # return direction
            #print("evade", direction)
            return direction
